...
public static class CbmToolCatalog
{
    ...
    private static void AppendVerbosityGuidance(StringBuilder builder)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("`search_graph`, `get_code_snippet`, `search_code`, and `get_architecture` accept `verbosity`: `full` (default, CBM-parity payloads) or `compact` (fewer tokens). Other tools are unchanged. Do not pass `compact` on every call, and do not call the same tool twice (`compact` then `full`). Choose once from the question type.");
        builder.AppendLine();
